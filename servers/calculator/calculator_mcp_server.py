...
async def get_calculator_info(ctx: Context) -> str:
    ...
    accessible_tools = get_accessible_tools(ctx)
    
    if hasattr(ctx, 'token_info') and ctx.token_info:
        user_info = f"User: {ctx.token_info.get('subject', 'unknown')}\nRoles: {', '.join(ctx.token_info.get('scopes', []))}"
    else:
